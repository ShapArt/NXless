...
public:
    BsdClientSession(
        socket::ClientContextId id,
        BsdForwarder& forwarder,
        socket::SocketRegistry& registry) noexcept;
    ~BsdClientSession();

    BsdClientSession(const BsdClientSession&) = delete;
    BsdClientSession& operator=(const BsdClientSession&) = delete;
    ...
    bool MarkProxyCandidateForTest(const int fd) noexcept {
        return registry_.SetTag(id_, fd, socket::InterceptionTag::ProxyCandidate);
    }
#endif

private:
    socket::ClientContextId id_{};
    BsdForwarder& forwarder_;
    socket::SocketRegistry& registry_;
    bool attached_{};
};
