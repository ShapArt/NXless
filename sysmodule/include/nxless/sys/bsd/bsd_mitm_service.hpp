...
public:
    ...
    ams::Result DuplicateSocket(ams::sf::Out<s32> out_ret,ams::sf::Out<s32> out_errno,s32 fd,u32 padding,u64 reserved) noexcept;
private:
...
#define AMS_NXLESS_BSD_MITM_INTERFACE(C,H) \
 ...
 AMS_SF_METHOD_INFO(C,H,27,ams::Result,DuplicateSocket,(ams::sf::Out<s32> out_ret,ams::sf::Out<s32> out_errno,s32 fd,u32 padding,u64 reserved),(out_ret,out_errno,fd,padding,reserved))
