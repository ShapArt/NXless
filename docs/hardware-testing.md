...
The record is tied to the source revision. Do not manually copy a record from another commit.

## 3. Produce the exact Switch build
...
Alternatively, if the package has already been built by the admitted environment, use the recorder's build path:

```sh
python3 scripts/phase0_hardware.py record-build --repo . --record evidence/phase0.json --builder "$USER"
```

The package verifier must accept `output/NXless-phase0.zip`, and the resulting SHA-256 must be stored in the evidence record.
...
2. Confirm `/config/nxless/` exists.
3. First test with `/config/nxless/disable.flag` present.
4. Never modify NAND or Nintendo system files for NXless recovery.

See `docs/recovery.md` for the recovery procedure.

## 5. Start the host echo endpoints
...
## 6. Build and run the test-only probe

Build `NXlessProbe.nro` from the same source revision. The probe is test-only and must not be added to the Phase 0 release ZIP.

Record both baseline-without-NXless and transparent-MITM results:

```sh
python3 scripts/phase0_hardware.py record-network --record evidence/phase0.json --protocol tcp --target 192.0.2.10:5001 --concurrent 1 --baseline pass --nxless pass
python3 scripts/phase0_hardware.py record-network --record evidence/phase0.json --protocol udp --target 192.0.2.10:5002 --concurrent 1 --baseline pass --nxless pass
```

Replace the documentation-only address above with the real LAN address used during the test.

## 7. Recovery and cold-boot matrix
...
Do not collapse repeated trials into one entry.
...
## 12. Generate the verdict

```sh
python3 scripts/phase0_hardware.py check --record evidence/phase0.json --level phase0 --markdown evidence/phase0.md
```

A passing result must be attached to issue #3 together with the JSON record, Markdown report, package SHA-256, and the exact source revision. An anecdotal "it boots" result is not sufficient.

## Failure handling

If any boot, network, lifecycle, or recovery step fails:

1. keep the failed evidence entry;
2. restore `disable.flag` or remove only `/atmosphere/contents/0100000000004E58` while powered off;
3. collect sanitized diagnostics;
4. reproduce before changing code;
5. fix through the normal test/review/CI path;
6. produce a new package and a new evidence record tied to the new commit.
