...
class HorizonSourceContractTests(unittest.TestCase):
    def test_accept_reinterprets_auto_select_buffer_as_bytes(self):
        self.assertIn('reinterpret_cast<std::byte*>(a.GetPointer())', SERVICE)
        self.assertNotIn('static_cast<std::byte*>(a.GetPointer())', SERVICE)
    ...
    def test_generated_sf_interfaces_check_implementations_at_compile_time(self):
        self.assertIn('static_assert(nxless::sys::bsd::IsIBsdMitmService<nxless::sys::bsd::BsdMitmService>)', SERVICE_HPP)
        control_hpp = (ROOT / 'sysmodule/include/nxless/sys/ipc/control_service.hpp').read_text(encoding='utf-8')
        self.assertIn('static_assert(nxless::sys::ipc::IsIControlService<nxless::sys::ipc::ControlService>)', control_hpp)
